...
OUTPUT="tmp"
MODELS_DIR="${CMSSW_BASE}/src/PhysicsTools/PyTorch/models"
...
bash $CMSSW_BASE/src/PhysicsTools/PyTorch/scripts/pytorch_aot_rename.sh .
echo "OK - Unzipping and preparing model"

# compilation with cmssw
if [ "${TARGET}" == "cpu" ]; then
  echo "Compilation for CPU backend"
  bash "$CMSSW_BASE"/src/PhysicsTools/PyTorch/scripts/pytorch_aot_compile.sh
  bash "$CMSSW_BASE"/src/PhysicsTools/PyTorch/scripts/pytorch_aot_link.sh
elif [ "${TARGET}" == "cuda" ]; then
  echo "Compilation for CUDA backend"
  bash "$CMSSW_BASE"/src/PhysicsTools/PyTorch/scripts/pytorch_aot_compile_cuda.sh
  bash "$CMSSW_BASE"/src/PhysicsTools/PyTorch/scripts/pytorch_aot_link_cuda.sh
fi
...
mkdir -p "${MODELS_DIR}"
mv "${MODEL}_${TARGET}_${ARCH}".pt2 "${MODELS_DIR}"
cd ../
echo "OK - Moving library to models registry"
