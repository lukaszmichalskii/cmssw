...
namespace ALPAKA_ACCELERATOR_NAMESPACE::torchtest {
  ...
  public:
    ...
    void produce(device::Event &event, const device::EventSetup &event_setup) override {
      Nvtx produce_range(fmt::format("Reco::produce(event: {}, stream: {}, device: {}, queue: {})",
                                     event.id().event(),
                                     static_cast<int>(event.streamID().value()),
                                     formatDevice(event.device()),
                                     QueueHash<Queue>::alpakaQueue(event.queue()))
                             .c_str());
      auto timer = EventTimer(fmt::format("ReconstructionMergeProducer ({})", kAlpakaBackend), event, verbose_);

      auto &regression_collection = event.get(regression_token_);
      auto &classification_collection = event.get(classification_token_);
      auto collection =
          ReconstructionDeviceCollection(regression_collection.const_view().metadata().size(), event.queue());
