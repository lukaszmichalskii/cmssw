...
namespace ALPAKA_ACCELERATOR_NAMESPACE::torchtest {
  ...
  void merge(Queue& queue, ReconstructionDeviceCollection &collection, const ClassificationDeviceCollection &classification, const RegressionDeviceCollection &regression) {
    ...
    alpaka::exec<Acc1D>(
        queue,
        grid,
        [] ALPAKA_FN_ACC(Acc1D const& acc, ReconstructionDeviceCollection::View reco_view, ClassificationDeviceCollection::ConstView cls_view, RegressionDeviceCollection::ConstView reg_view) {
          for (int32_t thread_idx : uniform_elements(acc, reco_view.metadata().size())) {
            reco_view[thread_idx].merged() = 
                cls_view[thread_idx].c1() * reg_view[thread_idx].reco_pt() + 
                cls_view[thread_idx].c2() * reg_view[thread_idx].reco_pt();
          }
        },
        collection.view(), classification.const_view(), regression.const_view());
  }
