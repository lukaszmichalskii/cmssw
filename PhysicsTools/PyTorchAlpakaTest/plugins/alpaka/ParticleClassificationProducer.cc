...
namespace ALPAKA_ACCELERATOR_NAMESPACE::torchtest {
  ...
  public:
    ...
    void produce(device::Event &event, const device::EventSetup &event_setup) override {
      Nvtx produce_range(fmt::format("Classification::produce(event: {}, stream: {}, device: {}, queue: {})",
                                     event.id().event(),
                             ...
                             .c_str());
      auto timer =
          EventTimer(fmt::format("ParticleClassificationProducer({})", kAlpakaBackend), event, verbose_);

      // in/out collections
      Nvtx alloc_range(fmt::format("Classification::malloc(event: {}, stream: {}, device: {}, queue: {})",
                                   event.id().event(),
                                   static_cast<int>(event.streamID().value()),
                                   formatDevice(event.device()),
                                   QueueHash<Queue>::alpakaQueue(event.queue()))
      ...
      Nvtx metadata_range(fmt::format("Classification::metadata(event: {}, stream: {}, device: {}, queue: {})",
                                      event.id().event(),
                                      static_cast<int>(event.streamID().value()),
                                      formatDevice(event.device()),
                                      QueueHash<Queue>::alpakaQueue(event.queue()))
      ...
      // inference
      {
        ...
        Nvtx torchlib_range(fmt::format("Classification::torchlib(event: {}, stream: {}, device: {}, queue: {})",
                                        event.id().event(),
                                        static_cast<int>(event.streamID().value()),
                                        formatDevice(event.device()),
                                        QueueHash<Queue>::alpakaQueue(event.queue()))
