...
namespace cms::torch::alpakatools {
    ...
    void copy(TQueue &queue, const MemcpyKind kind) {
        ...
        if constexpr (std::is_const_v<T>) {
          if (kind == MemcpyKind::DeviceToDevice)
            ...
            assert(false &&
                  "Unsupported MemcpyKind, only D2D copy operation is supported this backend.");
        }
  ...
  private:
    void deviceToDevice(TQueue& queue) {
      ...
      // copy data
      auto source_view = alpaka::createView(alpaka::getDev(queue), data_ptr_, alpaka::getExtents(dev_buffer_.value())[0]);
    ...
    void deviceToHost(TQueue& queue) {
      ...
      auto source_view = alpaka::createView(alpaka::getDev(queue), data_ptr_, alpaka::getExtents(host_buffer_.value())[0]);
    ...
    void hostToDevice(TQueue& queue) {
      ...
      if constexpr (!std::is_const_v<T>) {
        assert(host_buffer_ &&
               "HostBuffer not initialized! For ROCm/HIP CPU fallback avaliable only. Call deviceToHost first.");
        auto dest_view = alpaka::createView(alpaka::getDev(queue), data_ptr_, alpaka::getExtents(host_buffer_.value())[0]);
