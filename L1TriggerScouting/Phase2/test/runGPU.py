...
# process.gpu = process.scPhase2PuppiRawToDigiStruct.clone(
#     runCandidate = cms.bool(True),
#     runStructUnpacker = cms.bool(False),
#     runSOAUnpacker = cms.bool(False),
#     runGPU = cms.bool(True),
# )
process.cpu = process.scPhase2PuppiRawToDigiStruct.clone(
    runCandidate = cms.bool(True),
    runStructUnpacker = cms.bool(False),
    runSOAUnpacker = cms.bool(False),
    runGPU = cms.bool(False),
)

process.unpack_test = cms.Path(
   process.cpu
);

if options.outMode != "none":
  sched.append(getattr(process, "o_"+options.outMode))
process.schedule = cms.Schedule(process.unpack_test)
