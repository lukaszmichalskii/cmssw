...
class ScPhase2PuppiHPhiGammaDemo : public edm::stream::EDProducer<> {
public:
  explicit ScPhase2PuppiHPhiGammaDemo(const edm::ParameterSet &);
  ~ScPhase2PuppiHPhiGammaDemo() override;
...
ScPhase2PuppiHPhiGammaDemo::~ScPhase2PuppiHPhiGammaDemo(){};

void ScPhase2PuppiHPhiGammaDemo::beginStream(edm::StreamID) {
  countStruct_ = 0;
  passStruct_ = 0;
}
...
void ScPhase2PuppiHPhiGammaDemo::runObj(const OrbitCollection<T> &srcPuppi,
  ...
  for (unsigned int bx = 1; bx <= OrbitCollection<T>::NBX; ++bx) {
    ...
    // H mass
    auto mass = tripletmass(bestTriplet, cands, cands2, {{0.4937, 0.4937, 0.}});
  ...
  auto tab = std::make_unique<l1ScoutingRun3::OrbitFlatTable>(bxOffsets, "HPhiGamma" + label, true);
  ...
  iEvent.put(std::move(tab), "hphigamma" + label);
}
...
template <typename T>
bool ScPhase2PuppiHPhiGammaDemo::isolationTkEm(float pt, float eta, float phi, const T *cands, unsigned int size) const {
  bool passed = false;
  float psum = 0;
  for (unsigned int j = 0u; j < size; ++j) {  //loop over other particles
    float deta = eta - cands[j].eta(), dphi = ROOT::VecOps::DeltaPhi<float>(phi, cands[j].phi());
...
std::tuple<bool, float> ScPhase2PuppiHPhiGammaDemo::deltar(float eta1, float eta2, float phi1, float phi2) const {
  bool passed = true;
...
template <typename T>
float ScPhase2PuppiHPhiGammaDemo::pairmass(const std::array<unsigned int, 2> &t,
                                           const T *cands,
...
template <typename T, typename U>
float ScPhase2PuppiHPhiGammaDemo::tripletmass(const std::array<unsigned int, 3> &t,
                                              const T *cands,
                                              const U *cands2,
                                              const std::array<float, 3> &masses) {
...
DEFINE_FWK_MODULE(ScPhase2PuppiHPhiGammaDemo);
