  ...
  public:
    ...
    void analyze(edm::Event const& event, edm::EventSetup const&) override {
      ...
      if (puppi_handle.isValid() && bx_lookup_handle.isValid() && selected_bxs_handle.isValid() &&
        ...
        selected_bx_count_ += selected_bxs.const_view().metadata().size();
        // backends
    ...
    void logDebug(const SelectedBxHostCollection::ConstView selected_bxs, const int nbxs) {
      auto n_selected = selected_bxs.metadata().size();
      if (n_selected == 0)
        return;
    ...
    void logDebug(const W3PiHostTable::ConstView& table,
      ...
      for (int32_t idx = 0; idx < size; ++idx) {
        ...
        auto bx = 0;
        for (int32_t idx = 0; idx < bx_index.metadata().size(); idx++) {
          if (triplet.i() < offsets.offsets()[idx + 1] && triplet.i() >= offsets.offsets()[idx]) {
            bx = idx;
            break;
          }
        }
