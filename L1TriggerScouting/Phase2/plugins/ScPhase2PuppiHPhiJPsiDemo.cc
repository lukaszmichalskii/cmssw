...
class ScPhase2PuppiHPhiJPsiDemo : public edm::stream::EDProducer<> {
public:
  explicit ScPhase2PuppiHPhiJPsiDemo(const edm::ParameterSet &);
  ~ScPhase2PuppiHPhiJPsiDemo() override;
...
private:
  ...
  struct Cuts {
    ...
    float minmassH = 100;
    float maxmassH = 150;
    float minmassQ1 = 0.95;
    float maxmassQ1 = 1.25;
    float minmassQ2 = 2.50;
    float maxmassQ2 = 3.50;
...
ScPhase2PuppiHPhiJPsiDemo::ScPhase2PuppiHPhiJPsiDemo(const edm::ParameterSet &iConfig)
  ...
  if (doStruct_) {
    ...
    produces<l1ScoutingRun3::OrbitFlatTable>("hphijpsi");
  }
}
...
void ScPhase2PuppiHPhiJPsiDemo::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  if (doStruct_) {
...
void ScPhase2PuppiHPhiJPsiDemo::runObj(const OrbitCollection<T> &src,
  ...
  for (unsigned int bx = 1; bx <= OrbitCollection<T>::NBX; ++bx) {
    ...
    for (unsigned int i1 = 0; i1 < ndaus; ++i1) {
      ...
      for (unsigned int i2 = 0; i2 < ndaus; ++i2) {
        ...
        if (mass2 >= cuts.minmassQ1 and mass2 <= cuts.maxmassQ1)
          continue;
    ...
    // H mass
    auto mass = quadrupletmass(bestQuadruplet, cands, {{0.4937, 0.4937, 0.1057, 0.1057}});
  ...
  auto tab = std::make_unique<l1ScoutingRun3::OrbitFlatTable>(bxOffsets, "HPhiJPsi" + label, true);
  ...
  iEvent.put(std::move(tab), "hphijpsi" + label);
}
...
std::tuple<bool, float> ScPhase2PuppiHPhiJPsiDemo::deltar(float eta1, float eta2, float phi1, float phi2) const {
  bool passed = true;
...
DEFINE_FWK_MODULE(ScPhase2PuppiHPhiJPsiDemo);
