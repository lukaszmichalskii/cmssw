...
void ScPhase2PuppiHRhoGammaDemo::endStream() {
  if (doStruct_)
    std::cout << "HRhoGamma Struct analysis: " << countStruct_ << " -> " << passStruct_ << std::endl;
}

template <typename T, typename U>
void ScPhase2PuppiHRhoGammaDemo::runObj(const OrbitCollection<T> &src,
                                       const OrbitCollection<U> &src2,
                                       edm::Event &iEvent,
                                       unsigned long &nTry,
                                       unsigned long &nPass,
...
//TEST functions
template <typename T>
bool ScPhase2PuppiHRhoGammaDemo::isolationQ(
    unsigned int pidex1, unsigned int pidex2, const T *cands, unsigned int size) const {
  bool passed = false;
  float psum = 0;
