...
#error "Unable to detect backend type."
  ...
  L1TScPhase2RawToDigiKernels::L1TScPhase2RawToDigiKernels(Queue& queue) { initialize(queue); }
  ...
  void rawToDigi(Queue& queue, data_t* pf_data, PFCandidateCollection& pf_candidates) {
    ...
    uint32_t blocks_per_grid =
        cms::alpakatools::divide_up_by(pf_candidates.const_view().metadata().size(), threads_per_block);
    ...
    alpaka::exec<Acc1D>(
        ...
        [] ALPAKA_FN_ACC(Acc1D const& acc, OrbitEventIndexMapCollection::ConstView orbit_association_map) {
          if (cms::alpakatools::once_per_grid(acc)) {
            int32_t head = 10;
            int32_t span =
                (orbit_association_map.metadata().size() > head) ? head : orbit_association_map.metadata().size();
