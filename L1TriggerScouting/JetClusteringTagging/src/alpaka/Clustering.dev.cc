...
public:
  ...
  ALPAKA_FN_ACC void operator()(TAcc const& acc, PuppiCollection::ConstView data, ClustersCollection::View clusters, uint32_t clusters_num) const {
    ...
    for (uint32_t block_idx: independent_groups(acc, grid_dim)) {
      ...
      if (once_per_block(acc)) {
        ...
        alpaka::atomicAdd(acc, &clusters.density(), static_cast<uint32_t>(clusters_ptrack));
      }
      ...
      if (once_per_block(acc)) {
        for (uint32_t idx = 0; idx < block_dim - 1; idx++) {
          ...
          clusters.associations()[thread_idx] = mask[sorted_indices[idx]];
        }
      }
      ...
      if (once_per_block(acc)) {
        ...
        if (block_idx == max_id) {
          ...
          for (uint32_t idx = 0; idx < SHARED_MEM_BLOCK; idx++) {
            if (sorted_indices[idx] == -1)
              break;
            printf("|%2d -> %2d", sorted_indices[idx], clusters.associations()[sorted_indices[idx]+begin]);
          }
          printf("\n\n");
          ...
          for (uint32_t idx = 1; idx < clusters_num; idx++) {
            int accu = 0;
            for (uint32_t i = 0; i < block_dim; i++) {
              if (clusters.associations()[i+begin] == idx)
                accu += 1;
            }
            gaccu += accu;
            printf("|%8d", accu);
...
void SeededConeClustering::Cluster(
    Queue& queue, PuppiCollection const& data, ClustersCollection& clusters, uint32_t clusters_num) {
  uint32_t threads_per_block = ThreadsPerBlockUpperBound(128);
  uint32_t blocks_per_grid = data.view().bx().size();        
  auto grid = make_workdiv<Acc1D>(blocks_per_grid, threads_per_block);
  alpaka::exec<Acc1D>(queue, grid, SeededConeClusteringKernel{}, data.const_view(), clusters.view(), clusters_num);
  alpaka::wait(queue);
}
